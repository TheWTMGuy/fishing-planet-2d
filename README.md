...
pygame.display.set_caption("Fishing Simulator 2D - GitHub Edition")
...
# Game States: "HUB", "CASTING", "WAITING", "FIGHTING", "RESULT"
state = "HUB"
...
def perform_cast():
    ...
    state = "WAITING"
    cast_distance = (cast_power / 100) * 40 + 10 # 10 to 50 meters
    bite_timer = random.randint(120, 300) # 2 to 5 seconds

def hook_fish():
    global state, active_fish, fish_distance, fish_stamina, max_fish_stamina, line_tension
    state = "FIGHTING"
    
    # Filter fish by level
...
def update_fight():
    ...
    # AI Fish Behavior: Periodic pulling surges
    fish_surge_timer -= 1
    ...
    if reeling:
        ...
        line_tension -= 1.2 # Tension relaxes when not reeling
    ...
    if line_tension >= 100:
        ...
        result_color = RED
    elif line_tension <= 2 and reeling:
        # Too low tension while reeling leads to hook slip
        pass 
    elif line_tension <= 0:
        state = "RESULT"
...
def draw_scene():
    ...
    pygame.draw.polygon(screen, DARK_GREEN, [(0, 210), (190, 245), (150, 270), (0, 240)])
    
    # Angler Silhouette
    pygame.draw.line(screen, BLACK, (90, 210), (90, 170), 5) # Body
    pygame.draw.circle(screen, BLACK, (90, 162), 8) # Head
    pygame.draw.line(screen, BROWN, (90, 185), (140, 150), 4) # Rod
...
while running:
    ...
    if state == "HUB":
        ...
        # Stats / Controls info
        ctrl = font_small.render("Controls in Fight: [SPACE] Reel In | [UP/DOWN] Adjust Drag", True, WHITE)
